...
Erhalte den Problembericht des Nutzers. Initialisiere das `session_log.md`, falls nicht vorhanden.
...
```bash
gemini -i "Analysiere folgendes Problem und erstelle einen Plan in problem.md sowie ein Repro-Skript: <Nutzer-Prompt>" --skill issue-planner
```
Prüfe danach `.gemini_agents/prompts/problem.md`. Wenn die Analyse fehlt, wiederhole den Schritt oder brich mit Fehlerbericht ab.

#### Schritt B: Issue Fixer (Implementierung)
Starte den Fixer, um den Plan aus `problem.md` umzusetzen:
```bash
gemini -i "Setze den Plan aus .gemini_agents/prompts/problem.md um und verifiziere ihn mit dem Repro-Skript." --skill issue-fixer
```
